...
class SlotAttentionSmolensky(nn.Module):
    ...
    def __init__(
        ...
        if learnable_roles:
            ...
            self.role_embedding = nn.Parameter(torch.randn(1, num_roles, num_fillers) * num_fillers**-0.5)

            # hardcoding all dims to be 32 for now, will change later
            self.role_q = nn.Linear(num_fillers, 32)
            self.role_k = nn.Linear(feature_dim, 32)
            self.role_v = nn.Linear(feature_dim, 32)

            self.role_mlp = nn.Sequential(nn.Linear(32, self.role_dim),
                                          nn.ReLU(),
                                          nn.Linear(self.role_dim, self.role_dim))
    ...
    def forward(
        ...
        if self.learnable_roles:
            ...
            role_q = self.role_q(roles)
            role_k = self.role_k(inputs)
            role_v = self.role_v(inputs)

            role_dots = torch.einsum("bid,bjd->bij", role_q, role_k)
            role_attn = role_dots.softmax(dim=-1)
            roles = torch.einsum("bij,bjd->bid", role_attn, role_v)

            roles = self.role_mlp(roles)
